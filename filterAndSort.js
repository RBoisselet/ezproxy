...
function mark(selector) {
    ...
    if (index >= 0 && input.value.length > 0) { 
        innerHTML = 
            innerHTML.substring(0,index) + "<mark>"
            + innerHTML.substring(index,index+input.value.length)
            + "</mark>" + innerHTML.substring(index + input.value.length);
        inputText.innerHTML = innerHTML;
        }   
    });
